...
@IBDesignable public class TrimmerView: AVAssetTimeSelector {
    ...
    private var current_leftConstraint: CGFloat = 0
    private var current_rightConstraint: CGFloat = 0
    ...
    private func setupTrimmerView() {
        ...
        _leftConstraint = trimView.leftAnchor.constraint(equalTo: leftAnchor)
        _rightConstraint = trimView.rightAnchor.constraint(equalTo: rightAnchor)
        _leftConstraint?.isActive = true
        _rightConstraint?.isActive = true
    }
    ...
    @objc func handlePanGesture(_ gestureRecognizer: UIPanGestureRecognizer) {
        ...
        case .began:
            if isLeftGesture {
                current_leftConstraint = _leftConstraint!.constant
            } else {
                current_rightConstraint = _rightConstraint!.constant
            }
            updateSelectedTime(stoppedMoving: false)
        case .changed:
            let translation = gestureRecognizer.translation(in: superView)
            if isLeftGesture {
                update_leftConstraint(with: translation)
            } else {
                update_rightConstraint(with: translation)
            }
            layoutIfNeeded()
    ...
    private func update_leftConstraint(with translation: CGPoint) {
        let maxConstraint = max(rightHandleView.frame.origin.x - handleWidth - minimumDistanceBetweenHandle, 0)
        let newConstraint = min(max(0, current_leftConstraint + translation.x), maxConstraint)
        _leftConstraint?.constant = newConstraint
    }

    private func update_rightConstraint(with translation: CGPoint) {
        let maxConstraint = min(2 * handleWidth - frame.width + leftHandleView.frame.origin.x + minimumDistanceBetweenHandle, 0)
        let newConstraint = max(min(0, current_rightConstraint + translation.x), maxConstraint)
